...
// Field returns a "key=value" string styled for inline use inside log messages.
//
//	tlog.Step("Checking out %s", tlog.Field("branch", name))
func Field(key, value string) string {
	return colorMuted.Render(key+"=") + colorBold.Render(value)
}
